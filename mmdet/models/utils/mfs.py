...
class MFS:
    ...
    def _sampling_classes(self, sampled_num_classes, mean_score):
        # rank = dist.get_rank()
        # count = 0
        # for i in range(len(self.queue_gt_labels)):
        #     length = (self.queue_gt_labels[i] == i).sum()
        #     if length != 0:
        #         count += 1
        # print("rank{}:{}".format(rank, str(count)))
        mean_score_fg = mean_score[:self.num_classes]
        mean_score_revert = 1. / (mean_score_fg ** 4 + 10e-8)
        sample_probability = mean_score_revert / mean_score_revert.sum()
        # print(sample_probability)
        #
        # if rank == 1:
        #     f = open('/remote-home/share/ylzhang/fslt_complete/loce_mean_score_pro.txt', 'a')
        #     f1 = open('/remote-home/share/ylzhang/fslt_complete/loce_mean_score.txt', 'a')
        #     sp = sample_probability.cpu().numpy()
        #     mean_score_fg = mean_score_fg.cpu().numpy()
        #     for i in range(len(sp)):
        #         f.write(str(sp[i]) + ' ')
        #     f.write('\n')
        #     f.close()
        #     for i in range(len(mean_score_fg)):
        #         f1.write(str(mean_score_fg[i]) + ' ')
        #     f1.write('\n')
        #     f1.close()
